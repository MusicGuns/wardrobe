...
describe Wardrobe do
  ...
  describe '::from_file' do
    ...
    it "should sort cloth is correctly" do
      expect(clothes_from_path.clothes.map {|key,value| value.map {|value| value.type} }).to match_array([["головной убор","головной убор"],["штаны"]])
    end

    it "returns the number of cloth" do
        expect(clothes_from_path.clothes.map {|key, value| value}).to match_array([
        [have_attributes(type: "головной убор", name: "кепка", temperature: 5..20), have_attributes(type: "головной убор", name: "шапка", temperature: -20..0)],
